...
function buildDerivedElements(elements) {
  ...
  for (const srcEl of elements) {
    ...
    for (const col of srcEl.columns || []) {
      ...
      if (fm) dispName = fm[2];
      else if (col.name) dispName = String(col.name);
...
function convertAlteryxToSigma(xmlText, options = {}) {
  ...
  const rawNodes = root.Nodes?.Node || [];
  const rawConns = root.Connections?.Connection || [];
  const inputs = [], joins = [], formulas = [];
  const summarizes = [], selects = [], filters = [];
  const nodeMap = {};
  for (const node of rawNodes) {
    const toolId = node["@_ToolID"] || "";
    const plugin = node.GuiSettings?.["@_Plugin"] || "";
    const entryPoint = node.EngineSettings?.["@_EngineDllEntryPoint"] || "";
    const config = node.Properties?.Configuration;
    const metaFields = node.Properties?.MetaInfo?.RecordInfo?.Field || [];
    const info = { toolId, plugin, entryPoint, config, metaFields };
    nodeMap[toolId] = info;
    const cls = classifyAlteryxTool(plugin, entryPoint);
    gaps.push({
      toolId,
      plugin: plugin || entryPoint || "(unknown)",
      family: cls.family,
      kind: cls.kind,
      reason: cls.reason,
  ...
  for (const inp of inputs) {
    const fileStr = inp.config?.File?.["#text"] || inp.config?.File || "";
  ...
  if (factEl) {
    for (const form of formulas) {
      ...
      for (const ff of formulaFields) {
        ...
        if (sigmaFormula && !unmapped.length) {
          ...
          const fmt = inferSigmaFormat(sigmaFormula, dispName);
          const col = { id: colId, formula: sigmaFormula, name: dispName };
    ...
    for (const summ of summarizes) {
      const summFields = summ.config?.SummarizeFields?.SummarizeField || summ.config?.SummarizeField || [];
      const groupBy = summFields.filter((sf) => (sf["@_action"] || "") === "GroupBy");
      if (groupBy.length) {
        const keys = groupBy.map((sf) => sf["@_field"] || sf["@_rename"] || "?").join(", ");
        gaps.push({
          toolId: summ.toolId,
          plugin: summ.plugin || "Summarize",
          family: "summarize-groupby",
          kind: "dbt-offramp",
      ...
      for (const sf of summFields) {
        ...
        const action = sf["@_action"] || "";
        const rename = sf["@_rename"] || field;
        ...
        if (formula && !formula.startsWith("/*")) {
          ...
          let fmt = inferSigmaFormat(formula, metricName);
          if (fmt?.formatString === ",.2%") fmt = { kind: "number", formatString: ",.2f", suffix: "%" };
          const metric = { id: sigmaShortId(), formula, name: metricName };
          if (fmt) metric.format = fmt;
          targetEl.metrics.push(metric);
          warnings.push(`Summarize "${rename}" \u2192 ${formula.slice(0, 60)}`);
        } else if (formula?.startsWith("/*")) {
          gaps.push({
            toolId: summ.toolId,
            plugin: summ.plugin || "Summarize",
            family: "summarize-action",
            kind: "dbt-offramp",
  ...
  for (const de of derivedEls) {
    ...
    if (srcBaseName && srcEl.relationships) {
      for (const rel of srcEl.relationships || []) {
        if (!rel.name) continue;
        const tgtEl = elements.find((e) => e.id === rel.targetElementId);
...
function alteryxFormulaToSigma(formula, warnings) {
  ...
  f = f.replace(/(\[[^\]]+\]|\b\w+\b)\s+IN\s*\(([^)]+)\)/gi, (_, expr, args2) => {
    const vals = args2.split(",").map((v) => v.trim());
...
var ALTERYX_LIT_RE = /'(?:[^']|'')*'/g;
function maskAlteryxLiterals(s) {
  const lits = [];
  let out2 = "";
  let i = 0;
  while (i < s.length) {
  ...
  return { masked: out2, lits };
}
function unmaskAlteryxLiterals(s, lits) {
  return s.replace(/\u0000(\d+)\u0001/g, (_m, i) => {
    const inner = lits[Number(i)].slice(1, -1).replace(/''/g, "'").replace(/"/g, '\\"');
    return `"${inner}"`;
  });
}
function alteryxIfToSigma(f) {
...
function alteryxSummarizeToSigma(action, fieldDisplayName) {
  ...
  const map = {
    ...
    Max: `Max(${c})`,
    Count: "Count()",
    CountDistinct: `CountDistinct(${c})`,
    CountNonNull: `CountIf(IsNotNull(${c}))`,
    CountNull: `CountIf(IsNull(${c}))`,
    First: `First(${c})`,
    Last: `Last(${c})`,
    Concat: `ListAgg(${c})`
  };
...
function alteryxExtractPath(fileStr, dbOverride, schOverride) {
...
function detectUnmappedAlteryxFuncs(maskedFormula) {
  const found = [];
...
function classifyAlteryxTool(plugin, entryPoint) {
  const p = `${plugin} ${entryPoint}`;
  const dbt2 = (model) => `Port this tool to a dbt model (${model}) and point Sigma at the materialized table. Do not fake it as a Sigma calc column.`;
  ...
  if (/JoinMultiple/i.test(p)) {
    return { kind: "dbt-offramp", family: "join-multiple", reason: "Join Multiple is a multi-input ETL join, not a Sigma N:1 relationship.", dbtHint: dbt2("join of 3+ tables") };
  }
  ...
  if (/MultiFieldFormula|MultiRowFormula/i.test(p)) {
    return { kind: "dbt-offramp", family: "multi-formula", reason: "Multi-Field / Multi-Row Formula is windowed/row-wise ETL, not a Sigma calc column.", dbtHint: dbt2("window function or UPDATE across columns") };
  }
  ...
  if (/Union/i.test(p)) {
    return { kind: "dbt-offramp", family: "union", reason: "Union stacks rows \u2014 Sigma relationships do not UNION.", dbtHint: dbt2("UNION ALL of the input tables") };
  }
  if (/AppendFields|Append\./i.test(p)) {
    return { kind: "dbt-offramp", family: "append", reason: "Append Fields is a cartesian/row-bind, not a Sigma relationship.", dbtHint: dbt2("cross join or UNION") };
  }
  if (/CrossTab|Crosstab/i.test(p)) {
    return { kind: "dbt-offramp", family: "crosstab", reason: "Cross Tab pivots rows to columns \u2014 Sigma has no DM pivot source.", dbtHint: dbt2("dbt_utils.pivot / CASE pivot") };
  }
  if (/Transpose/i.test(p)) {
    return { kind: "dbt-offramp", family: "transpose", reason: "Transpose unpivots columns to rows.", dbtHint: dbt2("dbt_utils.unpivot") };
  }
  if (/\.Unique\.|AlteryxUnique/i.test(p)) {
    return { kind: "dbt-offramp", family: "unique", reason: "Unique is a DISTINCT grain change.", dbtHint: dbt2("SELECT DISTINCT ...") };
  }
  if (/\.Sample\.|RandomSample/i.test(p)) {
    return { kind: "dbt-offramp", family: "sample", reason: "Sample/Random is a row subset for ETL, not a Sigma filter.", dbtHint: dbt2("TABLESAMPLE / QUALIFY ROW_NUMBER") };
  }
  if (/GenerateRows|GenerateRow/i.test(p)) {
    return { kind: "dbt-offramp", family: "generate-rows", reason: "Generate Rows explodes the grain.", dbtHint: dbt2("GENERATOR / a date spine") };
  }
  if (/TextToColumns/i.test(p)) {
    return { kind: "dbt-offramp", family: "text-to-columns", reason: "Text To Columns splits a field into columns or rows.", dbtHint: dbt2("SPLIT_PART / FLATTEN") };
  }
  if (/RunningTotal/i.test(p)) {
    return { kind: "dbt-offramp", family: "running-total", reason: "Running Total is a windowed ETL column, not a Sigma metric.", dbtHint: dbt2("SUM() OVER (ORDER BY ...)") };
  }
  if (/RecordID|RecordId/i.test(p)) {
    return { kind: "dbt-offramp", family: "record-id", reason: "Record ID is a surrogate key generated at ETL time.", dbtHint: dbt2("ROW_NUMBER() OVER ()") };
  }
  if (/FindReplace|FuzzyMatch|Imputation|Tile\./i.test(p)) {
    return { kind: "dbt-offramp", family: "cleanup", reason: "Find Replace / Fuzzy Match / Imputation / Tile are data-prep, not semantic-layer logic.", dbtHint: dbt2("cleaning model") };
  }
  ...
  if (/In-?DB|InDB|AlteryxInDB/i.test(p)) {
    return { kind: "dbt-offramp", family: "in-db", reason: "In-DB tools already are warehouse SQL \u2014 promote that SQL to dbt, do not re-encode it as Sigma formulas.", dbtHint: dbt2("the In-DB SQL as a model") };
  }
  if (/Macro|DynamicInput|Download|API |Python|AlteryxRPlugin|RunCommand|Command\.|Blob/i.test(p)) {
    return { kind: "dbt-offramp", family: "script-macro", reason: "Macros / Python / R / Download / Dynamic Input have no Sigma equivalent.", dbtHint: dbt2("scripted model or an external job that lands a table") };
  }
...
function sqlCaseToIf(expr) {
  ...
  const result = masked.replace(caseRe, (_, inner) => {
    ...
    let result2 = elsePart;
    for (let i = whens.length - 1; i >= 0; i--) {
      result2 = `If(${whens[i][0]}, ${whens[i][1]}, ${result2})`;
    }
    return result2;
  });
  return unmaskDoubleQuotedForCase(result, lits);
}
var CASE_DQ_LIT_RE = /"(?:[^"\\]|\\.)*"/g;
function maskDoubleQuotedForCase(s) {
  const lits = [];
  let out2 = "";
  let i = 0;
  while (i < s.length) {
...
function unmaskDoubleQuotedForCase(s, lits) {
...
var res = convertAlteryxToSigma(xml, {
  connectionId: opt("connection", "<CONNECTION_ID>"),
  database: opt("database"),
  schema: opt("schema"),
  modelName: opt("name") || void 0
});
var payload = { dataModel: res.model, warnings: res.warnings, stats: res.stats, gaps: res.gaps || [] };
var text = JSON.stringify(payload, null, 2) + "\n";
var out = opt("out");
if (out) writeFileSync(out, text);
else process.stdout.write(text);
var dbt = (res.gaps || []).filter((g) => g.kind === "dbt-offramp");
var gapsOut = opt("gaps-out");
