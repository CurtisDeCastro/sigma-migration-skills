...
description: Convert a Streamlit model + dashboards into a Sigma data model and matching workbook. Discovery, calc translation, DM + workbook creation via REST, layout, and warehouse parity verification.
---
...
## Phase 0 — Assess (C1)
TODO: feature-gap scan + scope. Defer tenant inventory to the `streamlit-assessment` skill.

## Phase 1 — Discover (C2)
TODO: pull the Streamlit model + dashboard/report definitions and the warehouse columns.
...
## Phase 2 — Convert (C4)
TODO: Streamlit model → Sigma data-model JSON.
...
Compare Streamlit values vs Sigma (vs warehouse where possible). Gated by
`scripts/assert-phase6-ran.rb`. A migration is not done until parity is GREEN.

## Security: RLS / CLS (C9)
Detect Streamlit row-level/column-level security always; apply to Sigma
user-attributes + DM filters opt-in. TODO: document the Streamlit mechanism.
