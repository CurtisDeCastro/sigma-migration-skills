...
# BOUNDARY: domo-to-sigma workdirs never carry migrate-state.json in real
# usage (that's tableau-to-sigma's stamp) — this proves the SHARED script's
# fallback mechanics, not a domo-specific code path.
...
# BOUNDARY: LAYOUT_PHASE_BY_TOOL only registers 'tableau-to-sigma' — a genuine
# domo-to-sigma run-state.json (tool="domo-to-sigma") always hits the
# "no registered layout-phase key" SKIP, never this FAIL. Spoofing `tool` here
# contract-tests the SHARED script's sentinel mechanism directly (same script,
# same code path any adopting converter would hit), not domo-specific behavior.
...
#     domo-to-sigma, so this gate is an unconditional WARN no-op in this
#     plugin's copy of the shared script — nothing to assert.
#   - Gate 14 (visual-similarity floor): scripts/visual-similarity.py is not
#     vendored in domo-to-sigma either (the gate is entirely invisible when
