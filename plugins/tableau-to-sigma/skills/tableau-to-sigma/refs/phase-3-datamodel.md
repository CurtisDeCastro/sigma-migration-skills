...
> **Custom-SQL text is already XML-decoded by the converter — do NOT grep the `.twb` to "check" an operator.** Tableau stores custom-SQL `WHERE` clauses XML-escaped inside the `.twb`, and round-tripped/Windows-authored workbooks can double-escape them. The converter decodes both single- and double-escaped entities (plus numeric refs) to a fixed point, so the emitted `statement` always carries real operators (`<=`, `>=`, `<>`). If you ever see `&lt;` / `&gt;` / `&amp;`, or the human-eye artifacts **`<<` / `>>=`**, in a SQL statement, that is a **stored escape artifact of `<=` / `>=`** — normalize it in place and move on. Do NOT open the raw `.twb` to investigate entity double-encoding; it is a 10-minute dead end (the converter handles it — see `converter/PROVENANCE.json`).
