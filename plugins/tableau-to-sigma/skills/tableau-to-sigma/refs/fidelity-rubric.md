...
- **chart kinds + marks** — every viz is the source's kind (horizontal bar stays horizontal, KPI is a `kpi-chart` not a table, heatmap is not bars); marks/labels/reference lines carried; log scale present (mind the export-linear ceiling).
