...
5. **Node installed but invisible / no admin rights.** Version managers (fnm/nvm)
   ...
   probes the standard version-manager dirs and activates the install (persisting the
   PATH prepend); when node is genuinely absent it installs user-scoped (fnm/scoop) with
   a pinned LTS — never an unpinned "latest" download, never admin.
