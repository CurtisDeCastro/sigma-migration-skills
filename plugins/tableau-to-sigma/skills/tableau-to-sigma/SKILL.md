...
| 3 | Build the DM spec | author → `validate-spec.rb --type datamodel` | clean `dm-spec.json` + `join-plan.json` ledger (every synthesized Lookup / federated join probed unique via `probe-join-keys.rb` or resolved — final gate exit 23); any structural edit to source semantics (join drop / table collapse / filter rewrite) declared + proven equivalent via `probe-equivalence.rb` (`semantic-edits.json` — final gate exit 27, no waiver) | `refs/phase-3-datamodel.md` |
