...
then `--finalize`), `16` = pass 1 built + POSTed but `<workdir>/manual-residues.json`
carries `unbuilt` window/table-calc residue(s) a dashboard tile plots — the printed
checklist has the formula + Custom SQL skeleton + bind steps; build each, set
`status:"built"`, then `--finalize` (the final gate refuses GREEN on `unbuilt`
residues, exit 22; waiver `--accept-manual-residues "<calc,...>"`, budget-counted),
`4` = DM posted but the workbook layer needs an agent-authored spec — re-enter the gated spine with `--reuse-dm <id> --wb-spec <path>` (never hand-POST),
...
| 0c | Scope/cost sign-off | `estimate-cost.rb --workdir` (orchestrator-run after the gap scan, before any DM build) | `cost-estimate.json` (`confidence: rough`) + run-state ack (`cost_estimate_acknowledged`; missing ack at DM build = WARN this release) | `refs/phase-0-scope.md` |
...
| 3 | Build the DM spec | author → `validate-spec.rb --type datamodel` | clean `dm-spec.json` + `join-plan.json` ledger (every synthesized Lookup / federated join probed unique via `probe-join-keys.rb` or resolved — final gate exit 23) | `refs/phase-3-datamodel.md` |
...
> `"12,345B"`, never `12345`; `"(12.3%)"`, never `-0.123`). Minimum anchors —
...
>   "anchors": [
>     { "id": "a1", "panel": "TOP ACCOUNTS", "label": "United Widgets revenue",
>       "raw": "12,345B", "kind": "currency",
...
> Sigma element the value should land in. **Coverage rule: anchor EVERY tile.**
> An anchor only vouches for the tile it lands in — `verify-anchors.rb` measures
> per-displayed-tile coverage (`anchor_coverage` in the verdict) and, on the
> all-embedded (`charts_total==0`) anchors-ORACLE path, the final gate refuses
> substitution while any displayed tile has zero coverage. A tile that genuinely
> prints no anchorable value must be named HERE, at transcription time:
> `"coverage_waivers": [{ "tile": "<display name>", "reason": "<why>" }]`
> (top-level key in `source-anchors.json`). These anchors are the **measured value
...
render the full Sigma page and Read it against the source dashboard PNG.
