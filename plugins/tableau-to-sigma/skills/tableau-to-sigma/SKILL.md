...
| 5 | Build the workbook | `build-charts-from-signals.rb` → `post-and-readback` → `build-dashboard-layout.rb` → `put-layout.rb` | `preflight_lint` clean; **layout is the LAST write** (a bare spec PUT wipes it); `lod-audit.json` (exit 24) + `agg-semantics.json` (exit 26) resolved | `refs/phase-5-workbook.md`, `refs/chart-patterns.md`, `refs/element-kinds.md`, `refs/layout-grid.md` (5d), `refs/composition-recipe.md`, `refs/story-points.md` (stories) |
