...
| `scripts/hydrate-custom-sql.rb` | **Published-datasource (sqlproxy) hydration.** When a workbook connects to a *published* data source, the `.twb` carries only a `<connection class='sqlproxy'>` placeholder (no SQL) → the converter would emit a 0-column element and POST fails "Source not found". This splices the Custom SQL retrieved by `extract-custom-sql.rb` (which traverses INTO the published DS via Metadata GraphQL) into the `.twb` as a real `<relation type='text'>`, wrapping it and projecting output columns with upper-snake aliases (`"Sales Region" AS SALES_REGION`) so the emitted SQL is Snowflake-safe. The existing converter then builds a normal `kind:sql` element — no new DM-shaping code. `migrate-tableau.rb` runs this automatically before conversion whenever a sqlproxy datasource is detected (`HydrateCustomSql.twb_has_sqlproxy?`). Pure module + offline unit test `test-hydrate-custom-sql.rb`. |
