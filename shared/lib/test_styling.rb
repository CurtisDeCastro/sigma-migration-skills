...
check('styling emits only canonical Container/Element layout tags') do
  h = Styling.header(id: 'hdr', title: 'Dashboard', theme: Styling::DEFAULT_THEME)
  h[:layout].include?('<Container ') && h[:layout].include?('<Element ') &&
    !h[:layout].match?(/LayoutElement|GridContainer/)
end
